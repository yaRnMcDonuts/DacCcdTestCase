...
void main(){
    ...
    #if defined(NORMALMAP) || defined(PARALLAXMAP) || defined(AFFLICTIONNORMALMAP) 
...
#ifdef BARK
    alterBarkLiveliness(albedo, livelinessValue);
#else
    alterPlantLiveliness(albedo, livelinessValue);


#endif


albedo = desaturate(albedo, deathVar);
...
vec3 normal;
 ...
 float noiseHash = getStaticNoiseVar0(wPosition, afflictionValue);
    ...
    #ifdef AFFLICTIONALBEDOMAP
        #ifdef SIMARBOREAL_LEAVES
        //maybe interpolate to a new goopy/drippy affliction leaf texture eventually, but use alpha value cmpared to affliction value as a threshold to show more affliction parts of the affliction texture at full opacity
       //     newScaledCoords = mod(newTexCoord / m_AfflictionSplatScale, 1);
            afflictionAlbedo = texture2D(m_SplatAlbedoMap , newTexCoord);
            
         #elif defined(USE_TRIPLANAR_AFFLICTION_MAPPING)
    ...
    #ifdef AFFLICTIONNORMALMAP
        #ifdef SIMARBOREAL_LEAVES
            afflictionNormal = texture2D(m_SplatNormalMap , newTexCoord).rgb;
    ...
    float afflictionEmissiveIntensity = m_AfflictionEmissiveValue;
        ...
        afflictionValue = getAdjustedAfflictionVar(afflictionValue);
    #else
        noiseHash = 1;
    #endif        
...
// sunlightExposure and adjusted for nearby point/spot lights
...
#if defined(EMISSIVE) || defined (EMISSIVEMAP) || defined(AFFLICTIONTEXTURE)
    #ifdef EMISSIVEMAP
    ...
    gl_FragColor += emissive * pow(emissive.a, emissivePower) * emissiveIntensity;
#endif
