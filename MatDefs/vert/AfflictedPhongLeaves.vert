...
void main(){
   ...
   // ** on billboard type.   
        ...
        vec4 wPos = worldMatrix * modelSpacePos;   // NOTE: this was named wPosition before you renamed it to be compaitble with affliction shader. this way nosie cache happens with static wPosition, chayeah? i think
        wPosition = wPos.xyz;
        ...
        vec3 cameraOffset = wPos.xyz - g_CameraPosition;
        vDistance = length(cameraOffset);        
        vec3 dir = cameraOffset/vDistance; //normalize(wPos.xyz - g_CameraPosition);
        ...
        wPos.xyz += left * (corner.x - 0.5) * inSize;
        wPos.xyz += up * (corner.y - 0.5) * inSize;

        // Push it a little towards the camera (should maybe be a parameter)
        wPos.xyz += billboardNormal * 0.5;
